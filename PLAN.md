...
- L6 **CI gate `i18n:check`** runs `i18next-cli` in CI mode on every PR — fails the build if any source file uses a translation key missing from the `en` catalog. Mirrors the handbook CI gate from Slice 4b.
...
- **CI gate** — `pnpm i18n:check` runs `i18next-cli` in CI mode (`extract --dry-run --ci`); fails the build on any untranslated source string. New CI job wired into `.github/workflows/ci.yml`
